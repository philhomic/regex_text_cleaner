  ...
  //生成【替换操作】以上部分的分组按钮
	...
	$('#executeUserPlan').on('click', function(){
		if (userPlan.length == 0){
			layer.open({
			  title: '提示',
			  content: '目前没有设置任何用户策略'
			});
			return; //用户策略里面没东西，直接返回
		}
		var editor = $('#editor').get(0);
	...
	$('#batchFileExe').on('click', function(){
		...
		cleanFile('/Users/WangPei/Desktop/2.txt', 'gbk')
	})

	var cleanFile = function(filePath, encoding, savePath){
			if (!filePath){
				return;
			} else if (path.extname(filePath) !== '.txt'){
				layer.open({
				  title: '提示',
				  content: '请确保文本文件为txt格式'
				});
				return;
			}
			...
			if (!savePath || savePath == ''){
				savePath = path.format({
					root: '/ignored',
					dir: fileDir,
					base: 'cleaned_' + filename
				})
			}

			fs.readFile(filePath, (err, data) => {
				var cleaned_data = '';
				var linkedReplace = function(initText, rulesArr){
					var n = rulesArr.length;
					var iter = function(result, counter){
						if (counter > n) {
							return result;
						} else {
							return iter(result.replace(new RegExp(rulesArr[counter-1]['re'], rulesArr[counter-1]['flag']), rulesArr[counter-1]['f']), counter + 1);
						}
					}
					return iter(initText, 1)
				}
				console.log(data);
				var data = iconvLite.decode(data, encoding)
				console.log(data);
				cleaned_data = iconvLite.encode(linkedReplace(data, userPlan), encoding);

				fs.writeFile(savePath, cleaned_data, (err) =>{
					if (err) throw err;
					console.log('文件已保存');
				})
			})
	...
	var holder = document.getElementById('filelist');
	holder.ondragover = () => {
		return false;
	};
	holder.ondragleave = () => {
	...
	holder.ondrop = (e) => {
		...
		$entries = $('<ol class="entries"></ol>');
		$filelist.append($entries);
		for( var i = 0; i < length; i++){
			var entry = e.dataTransfer.items[i].webkitGetAsEntry();
			if(entry.isFile){
				console.log(entry);
				var file = $('<li class="fileType"><img src="img/file.png">' + entry.name + '</li>');
				$entries.append(file);
			} else if (entry.isDirectory){
				console.log(entry);
				var folder = $('<li class="fileType"><img src="img/folder.png">' + entry.name + '</li>')
